...
class WaterToC(Model):
    ...
    def __init__(self, 
        ...
        # Build DataCollector with global and local reporters
        reporters = {
        ...
        # Sample k water cells for local n reporting
        ...
        # Create and collect initial data
    ...
    # For each water cell, calculate local cooperation and apply local feedback
        for x in range(self.width):
            for y in range(self.height):
            ...
            # Get agents in neighborhood of this water cell (adjustable radius)
    ...
    def _get_coop_fraction(self):
        """Get cooperation fraction"""
        coop = self._count_cooperators()
        defect = self._count_defectors()
        total = coop + defect
